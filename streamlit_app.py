...
# --- ANA UYGULAMA ---
    ...
    if st.session_state.marka is None:
        ...
        # İŞTE BURASI KİRAL! EN ÜSTTE SEKMELER BAŞLIYOR
        tab_ana, tab_alarm = st.tabs(["📊 Ana Ekran", "🚨 Alarm Merkezi"])

        # --------------------------------------------------------------------------------
        # 🚨 ALARM MERKEZİ SEKME İÇERİĞİ (BAĞIMSIZ VE EN ÜSTTE)
        # --------------------------------------------------------------------------------
        with tab_alarm:
            ...
            with st.expander("👁️ Impressions Kurgusu", expanded=True):
                ...
                if imp_kural_aktif:
                    if 'ham_veri' in st.session_state and st.session_state.ham_veri is not None:
                        ...
                        st.warning("Arka planda veri bekleniyor... Lütfen önce Ana Ekran'dan raporu seçip verileri çek butonuna bas.")

        # --------------------------------------------------------------------------------
        # 📊 ANA EKRAN SEKME İÇERİĞİ
        # --------------------------------------------------------------------------------
        with tab_ana:
